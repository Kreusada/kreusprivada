...
class PublishCogs(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config = Config.get_conf(
            self, identifier=3924082348, force_registration=True
        )
    ...
    async def publishcog(self, ctx: commands.Context):
        ...
        cog = discord.utils.get(
            ctx.guild.roles, id=await self.config.guild(ctx.guild).cog_creator()
        )
        ...
        if not chan:
            return await ctx.send(
                "The channel has not yet been configured by the admins."
            )
        try:
            ...
            if pred is True:
                ...
                await ctx.author.send(
                    "What is the cog that you are wanting to publish today? Please use CamelCase."
                )
            else:
                return await ctx.send(
                    f"Okay {ctx.author.name}, I've canceled the publishing"
                )
        ...
        def repo_check(m):
            return (
                ...
                and m.content.startswith("https://github.")
            )
        ...
        if descr:
            ...
            try:
                description = await self.bot.wait_for(
                    "message", timeout=200, check=check
                )
            except asyncio.TimeoutError:
                return await ctx.author.send("You took too long to answer.")
        if reqs:
            await ctx.author.send(
                "Are there any pre-requirements needed for this cog? If not, type `None`."
            )
            try:
        ...
        if inst:
            ...
            try:
                repolink = await self.bot.wait_for(
                    "message", timeout=200, check=repo_check
                )
            except asyncio.TimeoutError:
                return await ctx.author.send(
                    "You took too long to provide a valid repository URL."
                )
        await ctx.author.send(
            "Perfect! We're all done. Check your guild's channel to see the results!"
        )
        if inst:
            instrepo = (
                f"`[p]repo add {repolink.content.split('/')[3]} {repolink.content}`"
            )
            instcog = (
                f"`[p]cog install {repolink.content.split('/')[3]} {cogname.content}`"
            )
        ...
        try:
            ...
            await ctx.author.send(
                f"Doesn't look like I have permission to post in {chan.mention}. I'm sorry!"
            )
    ...
    async def updatecog(self, ctx: commands.Context):
        ...
        try:
            ...
            if pred:
                ...
                await asyncio.sleep(1)
                await ctx.author.send(
                    "What is the cog that you are wanting to update today? Please use CamelCase."
                )
        except discord.Forbidden:
            return await ctx.send("I am not able to DM you.")
        ...
        try:
            ...
            return await ctx.author.send("You took too long to answer.")
        await ctx.author.send(
            "Please give a brief description of the changes you made. Markdown is supported."
        )
        try:
        ...
        if reqs:
            await ctx.author.send(
                "Let's remind users about prerequirements. If there isn't any, type `None`."
            )
            try:
                prereqs = await self.bot.wait_for("message", timeout=200, check=check)
            except asyncio.TimeoutError:
    ...
    async def cogcreator(self, ctx, role: discord.Role):
        ...
        await ctx.tick()
        await ctx.send(
            f"{role.mention} will now be considered as the Cog Creator role."
        )
    ...
    async def session_establishment(self, ctx: commands.Context, type: str):
        ...
        msg = await ctx.send(
            f"Are you sure you would like to {type}? I will send you some DMs."
        )
        pred = ReactionPredicate.yes_or_no(msg, ctx.author)
        start_adding_reactions(msg, ReactionPredicate.YES_OR_NO_EMOJIS)
        try:
